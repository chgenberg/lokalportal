...
function LoginContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result?.error) { setStep("error"); setSubmitting(false); setError("Felaktig e-post eller losenord"); }
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center px-4">
        <div className="w-full max-w-sm text-center animate-fade-in">
          ...
          <p className="text-[13px] text-gray-400">Oppna BankID-appen pa din enhet</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-white flex items-center justify-center px-4">
      <div className="w-full max-w-sm animate-fade-in">
        <div className="text-center mb-10">
          ...
          <p className="text-[13px] text-gray-400">Verifiera din identitet for att fortsatta</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => { setPassword(e.target.value); setError(""); }} required className="w-full px-4 py-3 bg-muted/50 rounded-xl text-sm border border-border/60 focus:border-navy/30 focus:bg-white outline-none transition-all pr-14" placeholder="Ditt losenord" />
        ...
        <div className="mt-8 pt-6 border-t border-border/40">
          <p className="text-[11px] text-gray-300 text-center leading-relaxed">
            Genom att logga in godkanner du vara <Link href="/villkor" className="text-navy/60 hover:underline">villkor</Link> och <Link href="/integritetspolicy" className="text-navy/60 hover:underline">integritetspolicy</Link>
          </p>
        </div>
      </div>
    </div>
