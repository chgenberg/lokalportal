...
type InputCategory = (typeof VALID_CATEGORIES)[number];
...
async function fetchSmhiWeather(lat: number, lng: number): Promise<string | null> {
  ...
  try {
    ...
    const tempParam = params.find((p) => p.name === "t" || p.name === "tcc_mean");
...
export async function POST(request: NextRequest) {
  ...
  let demographicsSummary: string | null = null;
  demographicsSummary = await fetchScbDemographics(city);
  ...
  try {
    ...
    if (!raw) {
      return NextResponse.json({ error: "Kunde inte generera annons" }, { status: 502 });
    }
    const parsed = JSON.parse(raw) as { title?: string; description?: string; tags?: string[] };
    title = String(parsed.title ?? "").trim().slice(0, 200) || "Kommersiell lokal";
