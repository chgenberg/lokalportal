      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex items-center justify-center gap-4 mb-14">
          <span
            className={`text-sm font-medium ${
              billingPeriod === "monthly" ? "text-navy" : "text-gray-400"
            }`}
          >
            Månadsvis
          </span>
          ...
          <span
            className={`text-sm font-medium ${
              billingPeriod === "yearly" ? "text-navy" : "text-gray-400"
            }`}
          >
            Årsvis
            <span className="ml-1.5 text-xs text-accent font-semibold">-20%</span>
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto">
          {packages.map((pkg) => (
            ...
            >
              {pkg.highlighted && (
                <div className="bg-accent text-white text-center py-2 text-xs font-semibold uppercase tracking-wider">
                  Mest populär
                </div>
              )}
              ...
              <div className="p-8">
                <div
                  className={`w-12 h-12 mb-4 rounded-xl flex items-center justify-center ${
                    pkg.highlighted ? "bg-accent/10" : "bg-muted"
                  }`}
                >
                  <pkg.icon
                    className={`w-6 h-6 ${
                      pkg.highlighted ? "text-accent" : "text-navy"
                    }`}
                  />
                </div>
        ...
        <div className="mt-20 max-w-3xl mx-auto">
          ...
          <div className="space-y-4">
            {[
              {
                q: "Hur fungerar BankID-verifieringen?",
                a: "Du loggar in med ditt BankID för att verifiera din identitet. Detta säkerställer att alla annonsörer är verifierade och ökar tryggheten för alla användare.",
              },
              {
                q: "Kan jag uppgradera mitt paket?",
                a: "Ja, du kan uppgradera när som helst. Du betalar bara mellanskillnaden för resterande period.",
              },
              {
                q: "Vad händer när min annons löper ut?",
                a: "Du får ett meddelande innan annonsen löper ut med möjlighet att förlänga. Utgångna annonser arkiveras och kan återaktiveras.",
              },
              {
                q: "Finns det någon bindningstid?",
                a: "Nej, det finns ingen bindningstid. Du betalar per period och kan avsluta när du vill.",
              },
            ].map((faq) => (
              <details
                key={faq.q}
                className="group bg-muted rounded-xl border border-border"
              >
